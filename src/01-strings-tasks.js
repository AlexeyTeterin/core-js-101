...
function encodeToRot13(s) {
  ...
  arr = arr.map((el) => {
    ...
    const notLetter = ' ?!.,'.indexOf(el) >= 0;
    const isSmall = el.charCodeAt(0) > 90;
    let res = '';
    let i = -1;

    if (notLetter) return el;
    i = alph.indexOf(el.toUpperCase()) + 13;
    if (i > 25) i -= 26;
    if (i === -1) return el;
    res = alph.charAt(i);
    return isSmall ? res.toLowerCase() : res;
  });
  return arr.join('');
}
...
function getCardId(card) {
  ...
  return values.indexOf(cardValue) + suits.indexOf(cardSuit) * 13;
}
